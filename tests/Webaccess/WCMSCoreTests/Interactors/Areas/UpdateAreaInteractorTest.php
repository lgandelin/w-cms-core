...
class UpdateAreaInteractorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testUpdateMasterArea()
    {
        ...
        Context::get('area_repository')->createArea($masterArea);

        $childArea1 = new Area();
        $childArea1->setID(2);
        $childArea1->setMasterAreaID($masterArea->getID());
        $childArea1->setName('Test area');
        Context::get('area_repository')->createArea($childArea1);

        $childArea2 = new Area();
        $childArea2->setID(3);
        $childArea2->setMasterAreaID($masterArea->getID());
        $childArea2->setName('Test area');
        Context::get('area_repository')->createArea($childArea2);
        ...
        $childArea1 = Context::get('area_repository')->findByID($childArea1->getID());
        $childArea2 = Context::get('area_repository')->findByID($childArea2->getID());
